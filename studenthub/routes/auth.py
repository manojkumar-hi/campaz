...
async def update_profile(
    ...
    if file:
        ...
        try:
            resp = requests.post(CLOUDINARY_UPLOAD_URL, files=files, data=data, timeout=10)
            resp.raise_for_status()
            update_data["profilePic"] = resp.json().get("secure_url")
        except requests.exceptions.Timeout:
            raise HTTPException(status_code=504, detail="Image upload timed out. Try a smaller image.")
